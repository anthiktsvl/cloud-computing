...
public class StationResource {
    ...
    @DELETE @Path("/{id}") public Response delete(@PathParam("id") int id, @Context HttpHeaders headers){ User u=AuthUtil.authenticate(headers); if(u==null)return AuthUtil.unauthorized(); if(!AuthUtil.isAdmin(u))return AuthUtil.forbidden(); try{ return dao.deleteStation(id)?Response.ok(new ApiResponse("Station deleted")).build():Response.status(404).build(); }catch(IllegalStateException e){ return Response.status(409).entity(new ApiResponse(e.getMessage())).build(); } }
}
