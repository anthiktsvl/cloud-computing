...
public class BookingResource {
    ...
    @DELETE @Path("/{id}") public Response cancel(@PathParam("id") int id, @DefaultValue("false") @QueryParam("hardDelete") boolean hardDelete, @Context HttpHeaders headers){ User u=AuthUtil.authenticate(headers); if(u==null)return AuthUtil.unauthorized(); try{ if(hardDelete){ if(!AuthUtil.isAdmin(u)) return AuthUtil.forbidden(); return dao.hardDeleteBooking(id,u)?Response.ok(new ApiResponse("Booking deleted")).build():Response.status(404).build(); } return dao.cancelBooking(id,u)?Response.ok(new ApiResponse("Booking cancelled")).build():Response.status(404).build(); }catch(SecurityException e){return AuthUtil.forbidden();}catch(IllegalStateException e){return Response.status(409).entity(new ApiResponse(e.getMessage())).build();} }
}
